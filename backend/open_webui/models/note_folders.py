...
class NoteFolderTable:
    def insert_new_folder(
        self, user_id: str, form_data: NoteFolderForm, parent_id: Optional[str] = None
        ...
        with get_db() as db:
            ...
            try:
                result = NoteFolder(**folder.model_dump())
                db.add(result)
                db.commit()
                db.refresh(result)
                if result:
                    return NoteFolderModel.model_validate(result)
                else:
                    return None
            except Exception as e:
                log.exception(f"Error inserting a new note folder: {e}")
                return None
    ...
    def get_folder_by_id_and_user_id(
        ...
        try:
            with get_db() as db:
                folder = db.query(NoteFolder).filter_by(id=id, user_id=user_id).first()
    ...
    def get_folders_by_user_id(self, user_id: str) -> list[NoteFolderModel]:
        with get_db() as db:
            return [
                NoteFolderModel.model_validate(folder)
                for folder in db.query(NoteFolder).filter_by(user_id=user_id).all()
            ]
        ...
        try:
            with get_db() as db:
                ...
                folder = (
                    ...
                    .filter(NoteFolder.name.ilike(name))
                    .first()
                )
    ...
    def get_folders_by_parent_id_and_user_id(
        ...
        with get_db() as db:
            return [
                NoteFolderModel.model_validate(folder)
                for folder in db.query(NoteFolder)
                .filter_by(parent_id=parent_id, user_id=user_id)
                .all()
            ]
    ...
    def update_folder_by_id_and_user_id(
        self, id: str, user_id: str, form_data: NoteFolderForm
    ) -> Optional[NoteFolderModel]:
        try:
            with get_db() as db:
                folder = db.query(NoteFolder).filter_by(id=id, user_id=user_id).first()
    ...
    def delete_folder_by_id_and_user_id(
        self, id: str, user_id: str, delete_notes=True
    ) -> bool:
        try:
            with get_db() as db:
                folder = db.query(NoteFolder).filter_by(id=id, user_id=user_id).first()
                ...
                if delete_notes:
                    ...
                    Notes.delete_notes_by_user_id_and_folder_id(user_id, folder.id)
                ...
                def delete_children(folder):
                    ...
                    for folder_child in folder_children:
                        if delete_notes:
                            Notes.delete_notes_by_user_id_and_folder_id(
                                user_id, folder_child.id
                            )

                        delete_children(folder_child)

                        folder = db.query(NoteFolder).filter_by(id=folder_child.id).first()
                        db.delete(folder)
                        db.commit()
