...
async def discover_custom_tools(request_type: str, config: Dict[str, Any]):
    log.info(f"Received custom MCP discovery request: type={request_type}")
    log.debug(f"Request config: {config}")
    ...
    if request_type == 'http':
        ...
        try:
            ...
            log.error(f"Error connecting to HTTP MCP server: {e}")
        ...
        try:
            ...
            log.error(f"Error connecting to SSE MCP server: {e}")
            raise HTTPException(status_code=400, detail=f"Failed to connect to MCP server: {str(e)}")
    else:
    ...
    log.info(f"Returning {len(tools)} tools for server {server_name}")
