<script lang="ts">
	...
	const documentClickHandler = (e) => {
		const searchContainer = document.getElementById('note-search-container');
		const noteSearch = document.getElementById('note-search');

		if (searchContainer && noteSearch && !searchContainer.contains(e.target) && !noteSearch.contains(e.target)) {
			if (e.target.id.startsWith('note-search-tag-') || e.target.id.startsWith('note-search-option-')) {
				return;
			}
			focused = false;
		}
...
<div class="px-1 mb-1 flex justify-center space-x-2 relative z-10" id="note-search-container">
	<div class="flex w-full rounded-xl" id="note-search">
			...
			on:keydown={(e) => {
				if (e.key === 'Enter') {
					if (filteredTags.length > 0) {
						const tagElement = document.getElementById(`note-search-tag-${selectedIdx}`);
						tagElement.click();
						return;
					}

					if (filteredOptions.length > 0) {
						const optionElement = document.getElementById(`note-search-option-${selectedIdx}`);
						optionElement.click();
						return;
					}
				}
				...
				if (!document.getElementById('note-search-options-container')) {
					onKeydown(e);
				}
			}}
		/>
	...
	{#if focused && (filteredOptions.length > 0 || filteredTags.length > 0)}
		...
		<div
			...
			id="note-search-options-container"
			in:fade={{ duration: 50 }}
			on:mouseenter={() => {
				selectedIdx = null;
			}}
						...
						{#each filteredOptions as option, optionIdx}
							<button
								...
								optionIdx
									? 'bg-gray-100 dark:bg-gray-900'
									: ''}"
								id="note-search-option-{optionIdx}"
								on:click|stopPropagation={async () => {
									const words = value.split(' ');
